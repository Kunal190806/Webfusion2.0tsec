          ...
          <div className={`footer-container ${footerClassName}`}>
            <div className={`footer-nav ${currentStep !== 1 ? 'spread' : 'end'}`}>
              ...
              <button onClick={isLastStep ? handleComplete : handleNext} className="next-button" {...nextButtonProps}>
                {isLastStep ? 'Complete Request' : nextButtonText}
              </button>
            </div>
          </div>
        )}
...
export function Step({ children }: any) {
  return <div className="step-default pb-8">{children}</div>;
}
...
function StepIndicator({ step, currentStep, onClickStep, disableStepIndicators }: any) {
  ...
  return (
    <motion.div onClick={handleClick} className="step-indicator" style={disableStepIndicators ? { pointerEvents: 'none', opacity: 0.5 } : {}} animate={status} initial={false}>
      <motion.div
        variants={{
          inactive: { scale: 1, backgroundColor: '#f4f3ef', color: '#a3a3a3' },
          active: { scale: 1, backgroundColor: '#16352F', color: '#16352F' },
          complete: { scale: 1, backgroundColor: '#16352F', color: '#16352F' }
        }}
...
function StepConnector({ isComplete }: any) {
  const lineVariants = {
    ...
    complete: { width: '100%', backgroundColor: '#16352F' }
  };
